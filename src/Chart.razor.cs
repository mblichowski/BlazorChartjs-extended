...
namespace PSC.Blazor.Components.Chartjs
{
	...
	public partial class Chart : IDisposable, IAsyncDisposable
	{
		...
		private IJSObjectReference _chartJsModuleRef;
		private DotNetObjectReference<IChartConfig> _dotNetObjectRef;

		protected override void OnInitialized()
		{
			_dotNetObjectRef = DotNetObjectReference.Create(Config);
		}
		...
		protected override async Task OnAfterRenderAsync(bool firstRender)
		{
			if (Config != null)
			{
				if (OldConfig == null || Config != OldConfig)
				{
					if (JSModule is null)
					{
						_chartJsModuleRef = await JSRuntime.InvokeAsync<IJSObjectReference>(
							 "import",
								 "./_content/PSC.Blazor.Components.Chartjs-extended/Chart.js");
						JSModule = new ChartJsInterop(_chartJsModuleRef);
					}

					await JSModule.Setup(_dotNetObjectRef, Config);
				}
		...
		{
			if (_chartJsModuleRef is not null)
				await _chartJsModuleRef.DisposeAsync();
